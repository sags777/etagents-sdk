...
export function register(program: Command): void {
  program
    ...
    .option(
      "--router <type>",
      "Routing strategy: triage (default) or rule",
      "triage",
    )
    .option(
      "--triage-model <id>",
      "Model for triage routing (default: agent's model)",
    )
      ...
      async (
        ...
        for (const agentPath of opts.agents) {
          try {
            ...
            console.error(
              `Error loading "${agentPath}": ${err instanceof Error ? err.message : String(err)}`,
            );
            process.exit(1);
          }
        }
        ...
        console.error(
          `Loaded ${agents.length} agents: ${agents.map((a) => a.name).join(", ")}`,
        );
        ...
        const router = AgentRouter.create().withStrategy(strategy);
