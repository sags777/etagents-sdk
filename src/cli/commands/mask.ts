...
export function register(program: Command): void {
  program
    .command("mask [text]")
    .description(
      "Mask PII in text using built-in or custom regex rules. Pipe-friendly.",
    )
    .option(
      "--rules <list>",
    ...
    .action(
      async (
        ...
        if (!input) {
          console.error(
            "Error: No input. Provide text as an argument or pipe via stdin.",
          );
          process.exit(1);
        }

        const enabledNames =
          opts.rules && opts.rules.length > 0
            ? opts.rules
            : BUILTIN_RULES.map((rule) => rule.name);

        const selectedCategories = BUILTIN_RULES.reduce<
        ...
        if (
          opts.rules &&
          !enabledNames.some((name) =>
            BUILTIN_RULES.some((rule) => rule.name === name),
          )
        ) {
