...
export function register(program: Command): void {
  program
    ...
    .description(
      "Scan a directory for agent files (*.agent.ts / *.agent.js) and MCP configs (*.mcp.json).",
    )
    ...
    .action(
      async (
        ...
        let result;
        try {
          result = ToolScanner.scan(directory, {
            agents: scanAgents,
            mcp: scanMcp,
          });
        } catch (err) {
        ...
        if (scanMcp) {
          if (result.mcpConfigs.length > 0) {
            ...
            for (const m of result.mcpConfigs) {
              const info = m.serverName
                ? ` — ${m.serverName} (${m.transport ?? "?"})`
                : "";
