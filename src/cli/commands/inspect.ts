...
import { agentToManifest } from "../../agent/agent-builder.js";
...
export function register(program: Command): void {
  program
    ...
    .option(
      "--store <spec>",
      "Store spec for session lookup: file:<dir> or redis:<url>",
      "file:.sessions",
    )
    ...
    .action(
      async (
        ...
        if (opts.json) {
          ...
          if (opts.sessionId) {
            out.session = await loadSession(
              opts.store ?? "file:.sessions",
              opts.sessionId,
            );
          }
        ...
        const prompt =
          manifest.systemPrompt.length > 120
        ...
        if (agent.mcp && agent.mcp.length > 0) {
          ...
          for (const m of agent.mcp) {
            const transport =
              "transport" in m
                ? (m as { transport: string }).transport
                : "stdio";
